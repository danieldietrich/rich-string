...
export = function s(staticParts: TemplateStringsArray, ...substitutions: unknown[]): string {
    const lines = substitutions                      // align substitutions and fuse them with static parts
        .map(val => String(val))
        .reduce((acc, subst, i) => acc + align(subst, acc) + staticParts[i + 1], staticParts[0])
    ...
    return lines
        ...
        .replace(/^(s*\n)+/, '');                    // removes leading whitespace while preserving indentation
};
